...
const requestURL = 'src/draft_data.json';
...
window.onload = function () {
  ...
  const displayManagers = () => { // DISPLAY MANAGER DATA
    ...
    managerStore.openCursor().onsuccess = (e) => {
      const cursor = e.target.result;
      if (cursor) {
        ...
        appendTable();
        addRows(table, cursor.value);
        displayNames(cursor.value, managerData);
        cursor.continue();
      } else {
  ...
  const draftPlayer = (draftBtn) => { // UPDATE DATA ON DRAFT BUTTON CLICK
    ...
    playerRequest.onsuccess = (e) => {
      ...
      updateManagerArrays(managerRequest, managerStore);
      updateSettings(settingsRequest, settingsStore);
      displayDrafted();
    };
  };
  ...
  const asyncDisplay = () => { // DISPLAY PLAYERS AND MANAGER AFTER GLOBALS RUN
    const info = displayInfo();
    const players = displayPlayers();
    const managers = displayManagers();
    const drafted = displayDrafted();
  };

  const asyncDraft = () => { // DISPLAY PLAYERS AND MANAGER AFTER GLOBALS RUN
    const info = displayInfo();
    const players = displayPlayers();
  };

  const clearDisplay = () => { // CLEAR STORES THEN RESET DISPLAY
    const clear = clearData();
    const display = asyncDisplay();
  };
